...
public class ShipmentServiceImpl implements ShipmentService {
    ...
    public Shipment createShipment(Long vehicleId, Shipment shipment) {
        ...
        locationRepository.findById(shipment.getPickupLocation().getId())
                .orElseThrow(() -> new ResourceNotFoundException("Pickup location not found"));
        locationRepository.findById(shipment.getDropLocation().getId())
        ...
        if (shipment.getScheduledDate().isBefore(LocalDate.now())) {
            throw new IllegalArgumentException("Cannot schedule shipment in past");
        }
